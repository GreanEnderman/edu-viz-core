...
export default function VolcanoPlot({ node }: { node: A2UINode }) {
  ...
  const initGeneNames = parseStr(props.geneNames, "");
  const initLog2FCs = parseStr(props.log2FCs, "");
  const initPValues = parseStr(props.pValues, "");
  ...
  const initComparison = parseStr(props.comparison, "");
  ...
  // 检查是否有数据
  const hasData = geneNames.trim() !== '' && log2FCs.trim() !== '' && pValues.trim() !== '';

  // 数据验证
  let validationError = '';
  if (hasData) {
    const geneArr = geneNames.split(',').map(s => s.trim()).filter(Boolean);
    const fcArr = log2FCs.split(',').map(s => s.trim()).filter(Boolean);
    const pArr = pValues.split(',').map(s => s.trim()).filter(Boolean);

    if (geneArr.length !== fcArr.length || geneArr.length !== pArr.length) {
      validationError = `数据长度不一致：基因名 ${geneArr.length} 个，FC 值 ${fcArr.length} 个，p 值 ${pArr.length} 个。三者数量必须相同。`;
    } else {
      const invalidFC = fcArr.some(v => isNaN(parseFloat(v)));
      const invalidP = pArr.some(v => {
        const num = parseFloat(v);
        return isNaN(num) || num < 0 || num > 1;
      });

      if (invalidFC) {
        validationError = '数据格式错误：FC 值必须是数字。';
      } else if (invalidP) {
        validationError = '数据格式错误：p 值必须是 0-1 之间的数字。';
      }
    }
  }

  const data = hasData && !validationError ? parseGeneData(geneNames, log2FCs, pValues, fcThreshold, pThreshold) : [];
      ...
      >
        <div
          style={{
            ...
            padding: 32,
            textAlign: "center",
            boxShadow: "0px 20px 40px rgba(27, 28, 26, 0.06)",
            maxWidth: 400,
          }}
        >
          <div
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <div style={{ fontWeight: 600, marginBottom: 4 }}>正确格式示例：</div>
            <div>• geneNames: "TP53,BRCA1,EGFR"</div>
            <div>• log2FCs: "2.5,-1.8,3.2"</div>
            <div>• pValues: "0.001,0.05,0.0001"</div>
          </div>
        </div>
      </div>
    );
