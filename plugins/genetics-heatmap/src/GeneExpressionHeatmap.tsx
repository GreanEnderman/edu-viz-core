...
export default function GeneExpressionHeatmap({ node }: { node: A2UINode }) {
  ...
  const genes = useMemo(() => parseGenes(rawGenes), [rawGenes]);
  const samples = useMemo(() => parseSamples(rawSamples), [rawSamples]);
  const normalizedData = useMemo(() => zScoreNormalize(genes), [genes]);
  ...
  useEffect(() => {
    ...
    normalizedData.forEach((row, i) => {
      row.forEach((zScore, j) => {
        ...
        ctx.strokeStyle = "rgba(197, 198, 207, 0.15)";
        ctx.lineWidth = 1;
